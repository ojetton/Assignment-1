...
```{r fig1}
ggplot(cool_data)+geom_point(aes(x=year, y=muniUse,col= 'Municiple Use'))+geom_point(aes(x=year, y=irrUse,col='Irrigation Use'))+geom_smooth(aes(x=year, y=muniUse),method = 'lm')+geom_smooth(aes(x=year, y=irrUse),method = 'lm')+labs(x='Year',y='Water Usage')
```
...
### Figure 2: S&P 500 
```{r data1}
setwd('//BENNS-PC/Users/Richard/Downloads')
...
```{r fig2}
library(scales)
ggplot(cooler_data)+geom_line(aes(x=Volume,y=Close,col='Close'))+geom_smooth(aes(x=Volume,y=Close,col='lm'),method ='lm')+geom_smooth(aes(x=Volume,y=Close,col='gam'),method = 'gam')+geom_smooth(aes(x=Volume,y=Close,col='loess'),method = 'loess')+labs(y='S&P 500 Closing Price')
...
### Figure 3: Precious Metals Prices
